# Modified version of https://github.com/rkadlec/asreader/blob/master/asreader/text_comprehension/cnn_ne_shuffle.py

import random
import numpy as np
from .reusable import Regenerator
...
def set_dictionary(dictionary):
    ...
    @Regenerator
    def find_en(dictionary):
        return filter(lambda word : word.find("@ent") == 0, dictionary)

    global named_entities
    named_entities = find_en(dictionary)

    @Regenerator
    def code_ne(named_entities):
        return map(lambda word : dictionary[word], named_entities)


    global named_entities_codes
    named_entities_codes = code_ne(named_entities)

def shuffle_ne(data):

    context, question, answer, candidates = data
    #shuffle the ne codes
    shuffled_ne = list(named_entities_codes)[:]
